...
class SharedAttentionProcessor:
    ...
    def shifted_scaled_dot_product_attention(self, attn: attention_processor.Attention, query: T, key: T, value: T) -> T:
        ...
        logits[:, :, :, query.shape[2]:] += self.score_shift
    ...
    def __call__(
        ...
        if not hasattr(attn, "attn_map"):
            ...
            attn.inference_step %= self.total_steps
        ...
        if not is_cross_attention and is_in_inference_step:# and self.share_attention:
...
def swapping_attentionv2(key, value, chunk_size=2, index=[0]):# [ref,content,inf,ref,content,inf]
    chunk_length = key.size()[0] // chunk_size  # [text-condition, null-condition] bf, chunk_size=2 -> bf b=3 f=2
    reference_image_index = index * chunk_length  # [0 0 0 0 0]
...
def swapping_attention_q(query, chunk_size=2, index=NotImplementedError, gamma=0.75):
    # latent_model_input: [ref,content,inf;uref,ucontent,uinf] if output_num is 1
    # from Style Injection in Diffusion, query from content, key and value from style
    chunk_length = query.size()[0] // chunk_size
    if index[0] >= chunk_length:
        return query
    content_image_index = index * chunk_length 
    query = rearrange(query, "(b f) d c -> b f d c", f=chunk_length)  
    # query = [:, [0, 1, 2]] -> [:, [0, 0, 0]] 
    query = gamma * query[:, content_image_index] + (1-gamma) * query
...
def swapping_attention_id(query, key, value, chunk_size=2, scale_1=None):
    chunk_length = key.size()[0] // chunk_size  # [text-condition, null-condition]

    s_idx, c_idx, cs_idx = 0, 1, 2  
    cs_idx = [cs_idx] * chunk_length
    s_idx = [s_idx] * chunk_length
    c_idx = [c_idx] * chunk_length
    # uncond & cond
    query = rearrange(query, "(b f) d c -> b f d c", f=chunk_length) 
    query[:,cs_idx] = scale_1 * query[:,c_idx] + (1-scale_1) * query[:,cs_idx] # 0 is s, 1 is c, 2 is cs
    query = rearrange(query, "b f d c -> (b f) d c")  

    # uncond & cond
    key = rearrange(key, "(b f) d c -> b f d c", f=chunk_length)
    key[:,cs_idx] = key[:,s_idx]  
    key = rearrange(key, "b f d c -> (b f) d c")

    # uncond & cond
    value = rearrange(value, "(b f) d c -> b f d c", f=chunk_length)
    value[:,cs_idx] = value[:,s_idx]  
    value = rearrange(value, "b f d c -> (b f) d c")

    return query, key, value


class MyCrossFrameAttnProcessor:
    def __init__(self, unet_chunk_size=2, attn_map_save_steps=[],activate_step_indices=None, total_steps=50,adain_queries=False, img_index=[0], swap_q=None):
        self.unet_chunk_size = unet_chunk_size
        self.attn_map_save_steps = attn_map_save_steps
        self.activate_step_indices = activate_step_indices
        self.total_steps = total_steps
    ...
    def __call__(
        ...
        is_cross_attention = encoder_hidden_states is not None # when it is true, text is k and v, latent is q
        ...
        query = attn.to_q(hidden_states) # num_of_seed, 64, 1280

        if encoder_hidden_states is None:
            encoder_hidden_states = hidden_states
        # elif attn.cross_attention_norm:
        #     encoder_hidden_states = attn.norm_cross(encoder_hidden_states)
        key = attn.to_k(encoder_hidden_states) # in cross-atten, it is condition: [num_of_seed, max_len_of_prompt, 1280]
        value = attn.to_v(encoder_hidden_states)
        ...
        if not is_cross_attention and is_in_inference_step:
            if self.img_index is not None and self.swap_q is None: 
                key, value = swapping_attentionv2(key, value, chunk_size=2, index=self.img_index)
                # key, value = swapping_attention(key, value)
            if self.swap_q is not None:
                query = 1.5 * swapping_attention_q(query,chunk_size=2,index=self.img_index)
                



        inner_dim = key.shape[-1]
        head_dim = inner_dim // attn.heads # heads is 20 in self-atten
...
# q -> tau*(gamma*adain(Q_cs) + (1-gamma)*adain(Q_c)), k -> K_s, v -> V_s
class MyCrossFrameAttnProcessor_v2:
    def __init__(self,
                 unet_chunk_size=2,
                 attn_map_save_steps=[],
                 style_activate_step_indices=None,
                 content_activate_step_indices=None,
                 total_steps=50,
                 style_img_index=None,
                 content_img_index=None,
                 adain_queries=False,
                 gamma=0.75,
                 tau=1.5):
        self.unet_chunk_size = unet_chunk_size
        self.attn_map_save_steps = attn_map_save_steps
        self.style_activate_step_indices = style_activate_step_indices
        self.content_activate_step_indices = content_activate_step_indices
        self.total_steps = total_steps
        self.adain_queries = adain_queries
        self.style_img_index = style_img_index
        self.content_img_index = content_img_index
        self.gamma = gamma
        self.tau = tau
    ...
    def __call__(
        ...
        style_is_in_inference_step = False
        content_is_in_inference_step = False

        if self.style_activate_step_indices is not None:
            for style_activate_step_index in self.style_activate_step_indices:
                if attn.inference_step >= style_activate_step_index[0] and attn.inference_step <= style_activate_step_index[1]:
                    style_is_in_inference_step = True
                    break

        if self.content_activate_step_indices is not None:
            for content_activate_step_index in self.content_activate_step_indices:
                if attn.inference_step >= content_activate_step_index[0] and attn.inference_step <= content_activate_step_index[1]:
                    content_is_in_inference_step = True
                    break

        if not is_cross_attention:
            if self.style_img_index is not None and style_is_in_inference_step: 
                key, value = swapping_attentionv2(key, value, chunk_size=2, index=self.style_img_index)
            if self.content_img_index is not None and content_is_in_inference_step:
                query = swapping_attention_q(query,chunk_size=2,index=self.content_img_index,gamma=self.gamma)
        ...
        if not is_cross_attention and (style_is_in_inference_step or content_is_in_inference_step):# and self.share_attention:
            if self.adain_queries:
                query = adain(query)

            hidden_states = nnf.scaled_dot_product_attention(
                query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False, scale=self.tau
            )
        else:
            hidden_states = nnf.scaled_dot_product_attention(
                query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
...
class MyCrossFrameAttnProcessor_id:
    def __init__(self, unet_chunk_size=2, attn_map_save_steps=[],activate_step_indices=None, total_steps=50,adain_queries=False, style_id=False, scale_1=0.75, scale_2=1.5, scale_in_cross=False):
        self.unet_chunk_size = unet_chunk_size
        self.attn_map_save_steps = attn_map_save_steps
        self.activate_step_indices = activate_step_indices
        self.total_steps = total_steps
        ...
        self.scale_in_cross = scale_in_cross

    def __call__(
            self,
            attn,
            hidden_states, # query
            encoder_hidden_states=None, # key, value
            attention_mask=None):
        ...
        if not is_cross_attention and is_in_inference_step:
            if self.style_id:
                query, key, value = swapping_attention_id(query, key, value, chunk_size=2, scale_1=self.scale_1)
                if not self.scale_in_cross:
                    query = self.scale_2 * query
            else:
        ...
        attention_probs = attn.get_attention_scores(query, key, attention_mask)
        if self.scale_in_cross:
            attention_probs *= self.scale_2
        if attn.inference_step in self.attn_map_save_steps:
            attn.attn_map[attn.inference_step] = attention_probs.clone().cpu().detach()
...
class CrossFrameAttnProcessor:
    def __init__(self, unet_chunk_size=2, attn_map_save_steps=[],activate_step_indices=None, total_steps=50,adain_queries=False):
        self.unet_chunk_size = unet_chunk_size
        self.attn_map_save_steps = attn_map_save_steps
        self.activate_step_indices = activate_step_indices
        self.total_steps = total_steps
...
# [0,4] [4,12] [12,18] [18,24] [24,30] [30,32]
ACTIVATE_LAYER_CANDIDATE_2_1 = [ 
        'down_blocks.0.attentions.0.transformer_blocks.0.attn1.processor',#0
        ...
        'down_blocks.0.attentions.1.transformer_blocks.0.attn2.processor',#3

        'down_blocks.1.attentions.0.transformer_blocks.0.attn1.processor',#4
        ...
        'down_blocks.2.attentions.1.transformer_blocks.0.attn2.processor',#11

        'up_blocks.1.attentions.0.transformer_blocks.0.attn1.processor', #12 形状 12-17
        ...
        'up_blocks.1.attentions.2.transformer_blocks.0.attn2.processor',#17

        'up_blocks.2.attentions.0.transformer_blocks.0.attn1.processor',#18 key position
        'up_blocks.2.attentions.0.transformer_blocks.0.attn2.processor', 
        'up_blocks.2.attentions.1.transformer_blocks.0.attn1.processor',#20
        ...
        'up_blocks.2.attentions.2.transformer_blocks.0.attn2.processor',#23

        'up_blocks.3.attentions.0.transformer_blocks.0.attn1.processor',#24 
        ...
        'up_blocks.3.attentions.2.transformer_blocks.0.attn2.processor',#29

        'mid_block.attentions.0.transformer_blocks.0.attn1.processor',#30
        'mid_block.attentions.0.transformer_blocks.0.attn2.processor',#31
...
ACTIVATE_LAYER_CANDIDATE= [
        ...
        'down_blocks.2.attentions.0.transformer_blocks.9.attn2.processor', #20 #28
        ...
        'down_blocks.2.attentions.1.transformer_blocks.9.attn2.processor',#20 #48
        ...
        'mid_block.attentions.0.transformer_blocks.9.attn2.processor', #20 #68
        ...
        'up_blocks.0.attentions.0.transformer_blocks.9.attn2.processor',#20 #88
        ...
        'up_blocks.0.attentions.1.transformer_blocks.9.attn2.processor',#20 #108
        ...
        'up_blocks.0.attentions.2.transformer_blocks.9.attn2.processor', #20 #128
        ...
        'up_blocks.1.attentions.2.transformer_blocks.1.attn2.processor',#12 #140
...
STYLE_DESCRIPTION_DICT = {
    ...
    "rain": ("Photography of {object}, realistic",""),
